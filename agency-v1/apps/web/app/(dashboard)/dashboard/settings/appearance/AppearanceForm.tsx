...
import { Palette, Moon, Sun, Monitor, Zap, AlignJustify, Type, Check, Loader2, Save } from "lucide-react";
...
export function AppearanceForm({ initialData }: AppearanceFormProps) {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        const toastId = toast.loading("Guardando preferencias en la nube...");
        ...
        try {
            ...
            if (result.success) {
                toast.success("Preferencias de apariencia guardadas con éxito en tu perfil", { id: toastId });
            } else {
        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="flex justify-end">
                <button onClick={handleSave} disabled={isSaving}
                    className="flex items-center gap-2 px-6 py-3 bg-[var(--ds-teal)] hover:bg-[var(--ds-teal-md)] text-white font-semibold rounded-[0.15rem] border border-[var(--ds-border-glow)] text-sm transition-all shadow-[var(--ds-shadow-teal)] disabled:opacity-50 cursor-pointer">
