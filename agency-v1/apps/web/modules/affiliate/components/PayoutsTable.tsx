...
export function PayoutsTable({ payouts }: Props) {
    ...
    return (
        <div className="rounded-sm overflow-hidden" style={{ border: '1px solid rgba(30,41,59,0.6)' }}>
            <div className="grid grid-cols-[1fr_auto_auto_1fr_auto] px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-slate-600 gap-4"
                style={{ background: 'rgba(15,23,42,0.9)', borderBottom: '1px solid rgba(30,41,59,0.6)' }}>
                <span>ID Pago</span>
                <span>Monto</span>
                <span>Estado</span>
                <span>Clave Idempotencia</span>
                <span>Fecha Pago</span>
            </div>
            {payouts.map((p, i) => (
                <div key={p.id}
                    className="grid grid-cols-[1fr_auto_auto_1fr_auto] px-4 py-3.5 items-center gap-4 hover:bg-slate-900/40 transition-colors"
                    ...
                    <span className="font-mono text-[10px] text-slate-600 truncate" title={p.idempotencyKey}>{p.idempotencyKey.slice(0, 20)}…</span>
