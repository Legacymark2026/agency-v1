...
export function PublicProposalClient({ proposal, onViewTracked }: PublicProposalClientProps) {
                    ...
                    <div className="flex items-center gap-5">
                        {currentProposal.company?.logoUrl ? (
                            <div className="h-16 w-16 bg-white rounded-xl p-2 shadow-[0_0_20px_rgba(255,255,255,0.1)]">
                                <img src={currentProposal.company.logoUrl} alt="Company Logo" className="h-full w-full object-contain" />
                            </div>
                        ) : (
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-white tracking-tight">{currentProposal.company?.name || "Agencia Asociada"}</h2>
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-12">
                            <div className="flex-1">
                                ...
                                <div className="space-y-2 text-slate-400">
                                    <p className="text-lg flex items-center gap-2">
                                        ...
                                        <strong className="text-slate-200">{currentProposal.contactName || currentProposal.contactEmail}</strong>
                                    </p>
                                    <p className="flex items-center gap-2">
                                        <Calendar className="h-4 w-4 text-slate-500" />
                                        <span>Emitido el {new Date(currentProposal.createdAt).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 min-w-[250px] shrink-0">
                                ...
                                <div className="text-3xl font-mono font-bold text-teal-400 flex items-end gap-2">
                                    ...
                                    <span className="text-sm font-sans font-medium text-slate-500 mb-1">{currentProposal.currency}</span>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tfoot className="bg-slate-900">
                                            <tr>
                                                ...
                                                <td className="px-8 py-6 text-right font-bold text-teal-400 text-2xl font-mono">
                                                    ${currentProposal.value.toLocaleString()} <span className="text-sm text-slate-500 font-sans">{currentProposal.currency}</span>
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
...
<button type="button" onClick={() => setShowSignature(false)} className="text-slate-400 hover:text-white">
                                    <X className="h-6 w-6" />
                                </button>
                            </div>
                            
                            <p className="text-slate-300 mb-6">Dibuja tu firma en el área siguiente:</p>
