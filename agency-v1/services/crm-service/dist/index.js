...
app.post("/api/leads", async (req, res) => {
    try {
        ...
        const assigneeId = await (0, assignment_engine_1.routeLead)(req.body);
        const leadData = {
            ...req.body,
...
app.post('/api/campaigns/sync', async (req, res) => {
    try {
        ...
        for (const c of campaigns) {
            ...
            const campaign = await database_1.prisma.campaign.upsert({
...
app.get('/api/campaigns/spend-stats', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const stats = await database_1.prisma.campaign.aggregate({
            where: { companyId: String(companyId) },
            _sum: {
                spend: true,
                impressions: true,
...
app.get('/api/campaigns/chart-data', async (req, res) => {
    try {
        ...
        const spends = await database_1.prisma.adSpend.groupBy({
            by: ['date'],
            where: {
                companyId: String(companyId),
                date: { gte: cutoff }
...
app.post('/api/crm/teams', async (req, res) => {
    try {
        ...
        const team = await database_1.prisma.team.create({ data: { name, companyId, parentId: parentId || null } });
...
app.post('/api/crm/custom-objects', async (req, res) => {
    try {
        ...
        const def = await database_1.prisma.customObjectDefinition.create({
...
app.get('/api/crm/automations/rules', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const rules = await database_1.prisma.dealAutomationRule.findMany({
            where: { companyId: String(companyId) },
...
app.post('/api/crm/automations/rules', async (req, res) => {
    try {
        ...
        const rule = await database_1.prisma.dealAutomationRule.create({
            data: {
                companyId,
                name,
                description,
...
app.patch('/api/crm/automations/rules/:id', async (req, res) => {
    try {
        const rule = await database_1.prisma.dealAutomationRule.update({
            where: { id: req.params.id },
            data: req.body
        });
...
app.delete('/api/crm/automations/rules/:id', async (req, res) => {
    try {
        await database_1.prisma.dealAutomationRule.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/crm/automations/logs', async (req, res) => {
    try {
        const log = await database_1.prisma.automationLog.create({
            data: req.body
        });
...
app.post('/api/crm/deals/:dealId/proposals', async (req, res) => {
    try {
        ...
        const deal = await database_1.prisma.deal.findUnique({ where: { id: req.params.dealId }, select: { companyId: true } });
        if (!deal)
            return res.status(404).json({ error: 'Deal not found' });
...
app.get('/api/crm/commissions/rules', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const rules = await database_1.prisma.commissionRule.findMany({
            where: { companyId: String(companyId) },
...
app.post('/api/crm/commissions/rules', async (req, res) => {
    try {
        ...
        const rule = await database_1.prisma.commissionRule.create({
            data: {
                companyId,
                userId: userId || null,
                rate: Number(rate),
...
app.patch('/api/crm/commissions/rules/:id', async (req, res) => {
    try {
        const rule = await database_1.prisma.commissionRule.update({
            where: { id: req.params.id },
            data: req.body
        });
        res.json({ success: true, data: rule });
...
app.delete('/api/crm/commissions/rules/:id', async (req, res) => {
    try {
        await database_1.prisma.commissionRule.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/crm/commissions/payments', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const payments = await database_1.prisma.commissionPayment.findMany({
            where: { companyId: String(companyId) },
            include: {
...
app.post('/api/crm/commissions/payments', async (req, res) => {
    try {
        ...
        const payment = await database_1.prisma.commissionPayment.create({
            data: {
                companyId,
                dealId,
                userId,
...
app.patch('/api/crm/commissions/payments/:id', async (req, res) => {
    try {
        const payment = await database_1.prisma.commissionPayment.update({
            where: { id: req.params.id },
            data: req.body
        });
...
app.get('/api/crm/goals', async (req, res) => {
    try {
        ...
        const goals = await database_1.prisma.salesGoal.findMany({
            where,
...
app.post('/api/crm/goals', async (req, res) => {
    try {
        ...
        let goal;
        if (level) {
            goal = await database_1.prisma.salesGoal.create({
                data: {
                    companyId,
                    level,
                    period,
            ...
            goal = await database_1.prisma.salesGoal.upsert({
                where: {
...
app.delete('/api/crm/goals/:id', async (req, res) => {
    try {
        await database_1.prisma.salesGoal.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/crm/sales/commissions/calculate', async (req, res) => {
    try {
        ...
        if (!companyId || !dealId)
            return res.status(400).json({ error: 'companyId and dealId required' });
        // Get Deal
        const deal = await database_1.prisma.deal.findUnique({
            where: { id: dealId },
        ...
        if (!deal || !deal.assignedTo)
            return res.status(404).json({ error: "Deal or owner not found" });
        if (deal.probability < 100)
            return res.json({ success: true, message: "Deal not WON yet" });
        // Get Commission Rule for the user (or global)
        const rule = await database_1.prisma.commissionRule.findFirst({
            where: {
                companyId,
                isActive: true,
        ...
        if (!rule)
            return res.status(400).json({ error: "No active commission rule found" });
        let rate = rule.rate;
        let type = "STANDARD";
        // Accelerator check
        const period = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}`;
        const userGoal = await database_1.prisma.salesGoal.findFirst({
        ...
        if (userGoal && userGoal.targetAmount > 0) {
            ...
            const wonDealsAgg = await database_1.prisma.deal.aggregate({
                where: {
                    companyId,
        ...
        const commission = await database_1.prisma.commissionPayment.create({
            data: {
                companyId,
                dealId: deal.id,
...
app.post('/api/crm/sales/commissions/clawback', async (req, res) => {
    try {
        ...
        if (!companyId || !dealId || !reason)
            return res.status(400).json({ error: 'companyId, dealId, and reason required' });
        const existing = await database_1.prisma.commissionPayment.findFirst({
            where: { dealId, companyId, status: { not: "CANCELLED" } }
        });
        if (!existing)
            return res.status(404).json({ error: "No commission found to clawback" });
        const clawback = await database_1.prisma.commissionPayment.create({
            data: {
                companyId,
                dealId,
        ...
        if (existing.status === "PENDING") {
            await database_1.prisma.commissionPayment.update({
...
app.post('/api/crm/audiences/calculate-ltv', async (req, res) => {
    try {
        ...
        const wonDeals = await database_1.prisma.deal.findMany({
            where: {
                companyId,
                stage: "WON",
        ...
        const midEmails = new Set();
        const lowEmails = new Set();
        ...
        const allRelatedLeads = await database_1.prisma.lead.findMany({
            where: {
                companyId,
        ...
        const updatePromises = allRelatedLeads.map(lead => {
            ...
            let tier = "LOW";
            if (highEmails.has(email))
                tier = "HIGH";
            else if (midEmails.has(email))
                tier = "MEDIUM";
            const leadData = { ...lead, ltvTier: tier };
            if (tier === "HIGH")
                highLeads.push(leadData);
            else if (tier === "MEDIUM")
                midLeads.push(leadData);
            else if (tier === "LOW")
                lowLeads.push(leadData);
...
app.get('/api/crm/sales/forecast', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const openDeals = await database_1.prisma.deal.findMany({
            where: {
                companyId: String(companyId),
...
app.get('/api/crm/sales/leaderboard', async (req, res) => {
    try {
        const { companyId, period } = req.query;
        if (!companyId || !period)
            return res.status(400).json({ error: 'companyId and period required' });
        const [year, month] = String(period).split("-").map(Number);
        const start = new Date(year, month - 1, 1);
        const end = new Date(year, month, 0, 23, 59, 59);
        const aggs = await database_1.prisma.deal.groupBy({
            by: ['assignedTo'],
            where: {
                companyId: String(companyId),
                stage: "WON",
        ...
        const leaderboard = await Promise.all(aggs.filter(a => a.assignedTo).map(async (agg) => {
            const u = await database_1.prisma.user.findUnique({
...
app.get('/api/crm/users/:userId/company', async (req, res) => {
    try {
        const cu = await database_1.prisma.companyUser.findFirst({
...
app.get('/api/crm/companies/:companyId/users', async (req, res) => {
    try {
        ...
        res.json({ success: true, data: users.map((u) => u.user) });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/api/crm/sequences', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const sequences = await database_1.prisma.emailSequence.findMany({
            where: { companyId: String(companyId) },
...
app.post('/api/crm/sequences', async (req, res) => {
    try {
        ...
        const seq = await database_1.prisma.emailSequence.create({
            data: {
                companyId,
                name,
                description,
...
app.patch('/api/crm/sequences/:id', async (req, res) => {
    try {
        const seq = await database_1.prisma.emailSequence.update({
            where: { id: req.params.id },
            data: req.body
        });
...
app.delete('/api/crm/sequences/:id', async (req, res) => {
    try {
        await database_1.prisma.emailSequence.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/crm/sequences/enroll', async (req, res) => {
    try {
        ...
        const sequence = await database_1.prisma.emailSequence.findUnique({ where: { id: sequenceId } });
        if (!sequence)
        ...
        const enrollment = await database_1.prisma.emailSequenceEnrollment.upsert({
...
app.patch('/api/crm/sequences/enrollments/:id', async (req, res) => {
    try {
        const enrollment = await database_1.prisma.emailSequenceEnrollment.update({
            where: { id: req.params.id },
            data: req.body
        });
        res.json({ success: true, data: enrollment });
...
app.get('/api/crm/sequences/due-enrollments', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const now = new Date();
        const due = await database_1.prisma.emailSequenceEnrollment.findMany({
            where: {
                status: 'ACTIVE',
                nextRunAt: { lte: now },
...
app.get('/api/crm/tasks/:id', async (req, res) => {
    try {
        const task = await database_1.prisma.task.findUnique({ where: { id: req.params.id } });
        if (!task)
...
app.post('/api/crm/email-templates', async (req, res) => {
    try {
        ...
        const template = await database_1.prisma.emailTemplate.create({
            data: {
                name,
                subject,
                body,
...
app.patch('/api/crm/email-templates/:id', async (req, res) => {
    try {
        const template = await database_1.prisma.emailTemplate.update({
            where: { id: req.params.id },
            data: { ...req.body, updatedAt: new Date() }
        });
...
app.delete('/api/crm/email-templates/:id', async (req, res) => {
    try {
        await database_1.prisma.emailTemplate.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/crm/scoring-rules', async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: 'companyId required' });
        const rules = await database_1.prisma.leadScoringRule.findMany({
            where: { companyId: String(companyId) },
            orderBy: { createdAt: "asc" },
        });
        res.json({ success: true, data: rules });
...
app.post('/api/crm/scoring-rules', async (req, res) => {
    try {
        ...
        const rule = await database_1.prisma.leadScoringRule.create({
...
app.get('/api/crm/scoring-rules/:id', async (req, res) => {
    try {
        const rule = await database_1.prisma.leadScoringRule.findUnique({ where: { id: req.params.id } });
        if (!rule)
...
app.patch('/api/crm/scoring-rules/:id', async (req, res) => {
    try {
        const rule = await database_1.prisma.leadScoringRule.update({
            where: { id: req.params.id },
            data: req.body
        });
        res.json({ success: true, data: rule });
...
app.delete('/api/crm/scoring-rules/:id', async (req, res) => {
    try {
        await database_1.prisma.leadScoringRule.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
function evaluateRule(value, operator, ruleValue) {
    switch (operator) {
...
async function computeLeadScore(lead, companyId) {
    const rules = await database_1.prisma.leadScoringRule.findMany({ where: { companyId, active: true } });
    let score = 0;
...
function enrichLeadWithEvents(lead) {
    const events = lead.marketingEvents || [];
    const aggregatedEvents = {
        website_visits: events.filter((e) => e.eventType === 'PAGE_VIEW').length,
        email_opens: events.filter((e) => e.eventType === 'EMAIL_OPEN').length,
        downloads: events.filter((e) => e.eventType === 'DOWNLOAD' || (e.eventName && e.eventName.toLowerCase().includes('descarga'))).length,
        webinars: events.filter((e) => e.eventType === 'WEBINAR' || (e.eventName && e.eventName.toLowerCase().includes('webinar'))).length,
        quote_requests: events.filter((e) => e.eventType === 'FORM_SUBMIT' && e.eventName && (e.eventName.toLowerCase().includes('cotiza') || e.eventName.toLowerCase().includes('presupuesto'))).length,
        demos: events.filter((e) => e.eventType === 'FORM_SUBMIT' && e.eventName && e.eventName.toLowerCase().includes('demo')).length,
        pricing_visits: events.filter((e) => e.eventType === 'PAGE_VIEW' && e.url && e.url.toLowerCase().includes('precio')).length,
    };
...
app.post('/api/crm/scoring/recalculate-all', async (req, res) => {
    try {
        ...
        for (const lead of leads) {
            ...
            const score = await computeLeadScore(enrichedLead, companyId);
            await database_1.prisma.lead.update({ where: { id: lead.id }, data: { score } });
            updated++;
        }
...
app.post('/api/crm/scoring/recalculate-lead', async (req, res) => {
    try {
        ...
        if (!leadId || !companyId)
            return res.status(400).json({ error: 'leadId and companyId required' });
        const lead = await database_1.prisma.lead.findUnique({
        ...
        const enrichedLead = enrichLeadWithEvents(lead);
        const score = await computeLeadScore(enrichedLead, companyId);
        await database_1.prisma.lead.update({ where: { id: leadId }, data: { score } });
...
app.get('/api/crm/reports', async (req, res) => {
    try {
        ...
        const [wonDeals, lostDeals, allLeads, allDeals, sourceStats] = await Promise.all([
            database_1.prisma.deal.findMany({ where: { companyId: String(companyId), stage: "WON" }, select: { value: true, createdAt: true, updatedAt: true } }),
            database_1.prisma.deal.findMany({ where: { companyId: String(companyId), stage: "LOST" }, select: { value: true, createdAt: true } }),
            database_1.prisma.lead.findMany({ where: { companyId: String(companyId) }, select: { source: true, status: true, score: true, createdAt: true } }),
            database_1.prisma.deal.findMany({ where: { companyId: String(companyId) }, select: { id: true, value: true, stage: true, source: true, createdAt: true, assignedUser: { select: { name: true } } } }),
            database_1.prisma.lead.groupBy({ by: ["source"], where: { companyId: String(companyId) }, _count: { source: true }, orderBy: { _count: { source: "desc" } } }),
        ]);
        // Revenue by month
...
app.get('/api/crm/automation/stagnant-deals', async (req, res) => {
    try {
        ...
        const stagnantDeals = await database_1.prisma.deal.findMany({
            where: {
                companyId: String(companyId),
...
app.get('/api/crm/deals/:dealId/stage-history', async (req, res) => {
    try {
        const history = await database_1.prisma.dealStageHistory.findMany({
            where: { dealId: req.params.dealId },
            orderBy: { createdAt: 'desc' },
...
app.get('/api/crm/closing/stagnant-deals', async (req, res) => {
    try {
        ...
        const stagnant = await database_1.prisma.deal.findMany({
            where: {
                companyId: String(companyId),
                stage: { notIn: ['WON', 'LOST'] },
...
app.get('/api/crm/closing/funnel-conversion-report', async (req, res) => {
    try {
        ...
        const stageCounts = await database_1.prisma.deal.groupBy({
            by: ['stage'],
            where: { companyId: String(companyId) },
            _count: { stage: true },
            _sum: { value: true },
        ...
        let avgDaysByStage = {};
        try {
            const historyData = await database_1.prisma.dealStageHistory.groupBy({
                by: ['toStage'],
