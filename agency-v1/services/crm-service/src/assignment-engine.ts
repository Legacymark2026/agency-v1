...
function evaluateCondition(lead: any, condition: Condition): boolean {
...
export async function routeLead(lead: any): Promise<string | null> {
  ...
  try {
      ...
      if (matches) {
        ...
        if (rule.roundRobinEnabled) {
          ...
          if (rule.teamId) {
            ...
            const companyMembers = await prisma.companyUser.findMany({
              where: { companyId },
              select: { userId: true },
            });
    ...
    const allCompanyMembers = await prisma.companyUser.findMany({
      where: { companyId },
      select: { userId: true },
    });
    const activeFallbackAgents = await prisma.user.findMany({
      where: {
