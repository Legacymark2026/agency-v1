...
const token_quota_service_1 = require("./token-quota.service");
const refrag_service_1 = require("./refrag.service");
const crm_variable_parser_service_1 = require("./crm-variable-parser.service");
const hitl_workflow_service_1 = require("./hitl-workflow.service");
const guardrails_service_1 = require("./guardrails.service");
const agent_governance_service_1 = require("./agent-governance.service");
const reasoning_trace_service_1 = require("./reasoning-trace.service");
...
class AiService {
     ...
     */
    static async getAgents(companyId) {
        try {
            return await database_1.prisma.agentConfig.findMany({
                where: { companyId },
     ...
     */
    static async runAgent(input) {
        const conversationId = input.conversationId || `conv-${Date.now()}`;
        // ── 📋 Start Reasoning Trace ───────────────────────────────────────────
        const tb = reasoning_trace_service_1.ReasoningTraceService.createBuilder(input.agentId, input.companyId, conversationId, input.userMessage);
        // ── 🎛️ PILAR 1: Governance Config (Autonomy Mode, Temperature, Thresholds) ──
        const governance = await agent_governance_service_1.AgentGovernanceService.getConfig(input.companyId, input.agentId);
        ...
        const inputGuardrail = guardrails_service_1.GuardrailsService.inspect(input.userMessage);
        if (!inputGuardrail.passed) {
            ...
            await reasoning_trace_service_1.ReasoningTraceService.saveTrace(tb.build());
        ...
        const quotaCheck = await token_quota_service_1.TokenQuotaService.checkQuota(input.companyId, input.agentId, 1500);
        ...
        const crmContext = await crm_variable_parser_service_1.CrmVariableParserService.loadContextFromDb(input.companyId, input.leadId);
        ...
        // ── 💾 Memoria Episódica ────────────────────────────────────────────────
        await agent_memory_service_1.AgentMemoryService.addMemory(input.agentId, conversationId, "user", cleanUserMessage);
        const memoryContext = await agent_memory_service_1.AgentMemoryService.getConversationContext(conversationId, 10);
        ...
        if (input.enableRefrag !== false) {
            refragResult = await refrag_service_1.RefragService.retrieveAndRerank(cleanUserMessage, input.companyId, {
                topK: 3,
            ...
            tb.setRefrag(refragResult.chunks.length, topScore)
                .addStep("REFRAG", `${refragResult.chunks.length} fragmentos recuperados (top score: ${topScore.toFixed(2)})`, refragResult.chunks.length > 0 ? "OK" : "WARN", { chunks: refragResult.chunks.map(c => ({ title: c.documentTitle, score: c.rerankedScore || c.score })) });
        }
        ...
        const isToolAllowed = (tool) => governance.allowedTools.length === 0 || governance.allowedTools.includes(tool);
        if (isToolAllowed("generate_quote") && (msgLower.includes("cotiza") || msgLower.includes("precio") || msgLower.includes("cuanto cuesta"))) {
            toolExecutionResult = await tool_executor_service_1.ToolExecutorService.executeTool({
            ...
            tb.addTool("generate_quote")
                .addStep("TOOL_CALL", "generate_quote ejecutado", toolExecutionResult.success ? "OK" : "WARN", { result: toolExecutionResult.result });
        }
            ...
            tb.addTool("search_crm")
                .addStep("TOOL_CALL", "search_crm ejecutado", toolExecutionResult.success ? "OK" : "WARN", { foundCount: toolExecutionResult.result?.foundCount });
        }
            ...
            tb.addTool("query_analytics")
                .addStep("TOOL_CALL", "query_analytics ejecutado", toolExecutionResult.success ? "OK" : "WARN", { metric: toolExecutionResult.result?.metric, value: toolExecutionResult.result?.value });
        }
        // ── 💬 Construir Respuesta ──────────────────────────────────────────────
        let rawResponse = "";
        ...
        if (toolExecutionResult?.success) {
            if (toolExecutionResult.toolName === "generate_quote") {
                ...
                rawResponse = `Hola {{user.name}}, encontré ${r.foundCount} coincidencias para {{lead.name}}: ${r.leads.map((l) => `${l.name} (${l.email})`).join(", ")}.`;
            }
        ...
        const parsedResponse = crm_variable_parser_service_1.CrmVariableParserService.parseVariables(rawResponse, crmContext);
        // Output guardrails
        const outputGuardrail = guardrails_service_1.GuardrailsService.inspect(parsedResponse);
        ...
        const hitlCheck = agent_governance_service_1.AgentGovernanceService.evaluateHitl(governance, confidenceScore, toolExecutionResult?.toolName, quoteAmount, cleanUserMessage);
        let pendingHitlItem = null;
        if (hitlCheck.requiresReview) {
            pendingHitlItem = await hitl_workflow_service_1.HitlWorkflowService.createPendingReview({
                agentId: input.agentId,
                companyId: input.companyId,
                conversationId,
        ...
        tb.setHitl(hitlCheck.requiresReview, hitlCheck.reason)
            ...
            : `Aprobado automáticamente (${governance.autonomyMode})`, hitlCheck.requiresReview ? "WARN" : "OK");
        // ── 💾 Persitir memoria + tokens ────────────────────────────────────────
        const tokensUsed = 280 + (governance.temperature > 0.7 ? 60 : 0);
        await agent_memory_service_1.AgentMemoryService.addMemory(input.agentId, conversationId, "assistant", safeResponse, {
            confidenceScore,
            hitlPending: !!pendingHitlItem,
            toolExecuted: toolExecutionResult?.toolName || null
        });
        await token_quota_service_1.TokenQuotaService.recordTokenUsage(input.companyId, input.agentId, tokensUsed, 160);
        ...
        await reasoning_trace_service_1.ReasoningTraceService.saveTrace(trace);
