...
exports.triageAndRouteMessage = triageAndRouteMessage;
...
const redis = new ioredis_1.default(REDIS_URL, { maxRetriesPerRequest: 3, lazyConnect: true });
...
// ─── AI Config ───────────────────────────────────────────────────────────────
async function getAIModelConfig(companyId) {
    const config = await database_1.prisma.integrationConfig.findFirst({
        where: { companyId, provider: { in: ["openai", "gemini", "anthropic", "deepseek", "mistral", "cohere"] } }
    });
    if (config?.provider === "openai")
        return { provider: "openai", apiKey: config?.config?.openaiApiKey || process.env.OPENAI_API_KEY };
    if (config?.provider === "anthropic")
        return { provider: "anthropic", apiKey: config?.config?.anthropicApiKey || process.env.ANTHROPIC_API_KEY };
    if (config?.provider === "deepseek")
        return { provider: "deepseek", apiKey: config?.config?.deepseekApiKey || process.env.DEEPSEEK_API_KEY };
    if (config?.provider === "mistral")
        return { provider: "mistral", apiKey: config?.config?.mistralApiKey || process.env.MISTRAL_API_KEY };
    const apiKey = config?.config?.geminiApiKey || process.env.GOOGLE_GENERATIVE_AI_API_KEY || process.env.GEMINI_API_KEY;
    if (!apiKey)
        throw new Error("API Key de AI no configurada.");
...
function injectCRMVariables(prompt, contactData) {
    const varMap = {
...
// ─── Sentiment Analyzer ─────────────────────────────────────────────────────
async function analyzeSentiment(message, companyId) {
...
function applyStyleFilter(text) {
...
async function isCircuitOpen(companyId) {
    try {
...
async function recordGeminiError(companyId) {
    try {
        const key = `cb:gemini:${companyId}`;
...
async function resetCircuitBreaker(companyId) {
    try {
        await redis.del(`cb:gemini:${companyId}`);
    }
    catch { /* non-fatal */ }
}
// ─── Human Transfer ─────────────────────────────────────────────────────────
async function triggerHumanTransfer(conversationId, reason, minutes, webhookUrl) {
    const suspendedUntil = new Date(Date.now() + minutes * 60 * 1000);
    await database_1.prisma.agentConversation.update({ where: { id: conversationId }, data: { status: "SUSPENDED", suspendedUntil, suspendedReason: reason } });
    if (webhookUrl) {
        try {
            await fetch(webhookUrl, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event: "HUMAN_TRANSFER_REQUESTED", conversationId, reason, suspendedUntil: suspendedUntil.toISOString() }), signal: AbortSignal.timeout(5000) });
        }
        catch (e) {
...
// ─── MAIN RUNNER ─────────────────────────────────────────────────────────────
async function runAIAgent({ agentId, companyId, userMessage, conversationId, senderUserId, contactData = {}, inlineHistory = [], userContext }) {
    const startTime = Date.now();
    const agent = await database_1.prisma.aIAgent.findUnique({ where: { id: agentId }, include: { knowledgeBases: { where: { isActive: true } } } });
    if (!agent || agent.companyId !== companyId)
        throw new Error("Agente no encontrado.");
    if (!agent.isActive)
        throw new Error(`El agente "${agent.name}" está inactivo.`);
    // Suspension check
    if (conversationId) {
        const conversation = await database_1.prisma.agentConversation.findUnique({ where: { id: conversationId } });
        if (conversation?.status === "SUSPENDED" && conversation.suspendedUntil && conversation.suspendedUntil > new Date()) {
            ...
            await database_1.prisma.agentConversation.update({ where: { id: conversationId }, data: { status: "ACTIVE", suspendedUntil: null } });
        }
        // Human-in-the-Loop
        if (agent.priorityAlpha && senderUserId) {
            const sender = await database_1.prisma.user.findUnique({ where: { id: senderUserId }, select: { role: true } });
    ...
    if (agent.enforceTempClamp)
        temperature = Math.min(0.5, Math.max(0.2, temperature));
    if (agent.enforceTokenLimit)
    ...
    if (inlineHistory.length > 0) {
        ...
        const prevMessages = await database_1.prisma.agentMessage.findMany({ where: { conversationId }, orderBy: { createdAt: "desc" }, take: 100 });
        let estimatedTokens = 0;
        const selected = [];
        for (const m of prevMessages) {
            const tokens = m.tokensUsed || Math.ceil(m.content.length / 4);
            if (estimatedTokens + tokens > 8000 && selected.length > 0)
                break;
    ...
    // Persist messages
    if (conversationId) {
        await database_1.prisma.agentMessage.create({ data: { conversationId, role: "user", content: userMessage, sentimentScore } });
        await database_1.prisma.agentMessage.create({ data: { conversationId, role: "assistant", content: finalResponse, rawContent: rawResponse !== finalResponse ? rawResponse : null, tokensUsed, latencyMs } });
    }
...
async function triageAndRouteMessage(companyId, userMessage, conversationId, contactData, inlineHistory, userContext) {
    const activeAgents = await database_1.prisma.aIAgent.findMany({ where: { companyId, isActive: true }, select: { id: true, name: true, description: true } });
    if (!activeAgents.length)
        return { result: "No hay agentes activos." };
    ...
    try {
        ...
        const { text } = await (0, ai_1.generateText)({ model: aiModel, prompt: routerPrompt });
        ...
        if (validAgents.length === 0)
            validAgents.push(activeAgents[0]);
        ...
        const { text: finalResult, usage } = await (0, ai_1.generateText)({ model: aiModel, prompt: synthPrompt });
...
async function disconnectRedis() { await redis.quit(); }
