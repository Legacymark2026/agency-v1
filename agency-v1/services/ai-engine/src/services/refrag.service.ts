...
export interface RefragChunk {
  ...
  content: string;
  score: number; // Score de relevancia de 0.0 a 1.0
...
export class RefragService {
   ...
   */
  static async retrieveAndRerank(
    query: string,
    companyId: string,
    options: RefragOptions = {}
  ): Promise<{ chunks: RefragChunk[]; compressedContext: string }> {
    ...
    try {
      rawDocuments = await (prisma as any).knowledgeBaseDocument.findMany({
        where: { companyId },
        take: 20
      });
    } catch {
    ...
    const candidateChunks: RefragChunk[] = [];
    ...
    for (const doc of rawDocuments) {
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        for (const token of queryTokens) {
          if (paragraphLower.includes(token)) matches++;
        }
