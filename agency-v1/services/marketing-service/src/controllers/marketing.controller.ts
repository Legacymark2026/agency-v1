...
export class MarketingController {
  ...
  static async generateCopy(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const copy = await MarketingService.generateAiCopy(companyId, topic, channel);
   ...
   */
  static async saveVisualCampaign(req: Request, res: Response, next: NextFunction) {
    try {
   ...
   */
  static async sendOmnichannel(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      let result;
      if (channel === "sms") {
        result = await MarketingService.sendSmsCampaign(companyId, name || "Campaña SMS", body, recipients);
      } else {
        result = await MarketingService.sendWhatsAppCampaign(companyId, name || "Campaña WhatsApp", body, recipients);
      }
