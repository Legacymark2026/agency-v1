...
export class ReportExportService {
   ...
   */
  static async generateCampaignReportHtml(blastId: string): Promise<string> {
    return `
      <!DOCTYPE html>
      <html>
        <head>
   ...
   */
  static async generateExecutiveSummary(companyId: string, dateRange?: { start: Date, end: Date }) {
    return {
      companyId,
      totalSent: 50000,
   ...
   */
  static async getContactTimeline(email: string, companyId: string) {
    const timeline = [
