...
class DomainReputationService {
    ...
    static async checkBlacklists(domain) {
        ...
        const results = [];
        try {
            const addresses = await dns_1.default.promises.resolve4(domain);
            if (addresses.length > 0) {
                ...
                for (const bl of blacklists) {
                    try {
                        ...
                        await dns_1.default.promises.resolve4(query);
     ...
     */
    static async checkDmarcDkimSpf(domain) {
        ...
        // DMARC
        try {
            const txtRecords = await dns_1.default.promises.resolveTxt(`_dmarc.${cleanDomain}`);
            const flatRecords = txtRecords.map((r) => r.join(""));
        ...
        // DKIM
        try {
            const txtRecords = await dns_1.default.promises.resolveTxt(`_domainkey.${cleanDomain}`);
            const flatRecords = txtRecords.map((r) => r.join(""));
     ...
     */
    static async getSenderScore(companyId) {
     ...
     */
    static getDomainWarmupSchedule(domain, dailyTarget) {
        const schedule = [];
     ...
     */
    static async getFullReputationReport(domain, companyId) {
        const blacklists = await this.checkBlacklists(domain);
        const auth = await this.checkDmarcDkimSpf(domain);
...
exports.DomainReputationService = DomainReputationService;
//# sourceMappingURL=domain-reputation.service.js.map