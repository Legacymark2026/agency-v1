...
class MarketingService {
     ...
     */
    static async sendSmsCampaign(companyId, name, body, recipients) {
        ...
        try {
            if (!require('fs').existsSync('./renders'))
                require('fs').mkdirSync('./renders');
            require('fs').writeFileSync(logFile, logContent, 'utf8');
        }
        catch (e) {
            console.warn("[MarketingService] SMS log write skipped:", e);
        }
        await eventBus.publish("sms.campaign_completed", {
            companyId,
            name,
     ...
     */
    static async sendWhatsAppCampaign(companyId, name, body, recipients) {
        ...
        await eventBus.publish("whatsapp.campaign_completed", {
            companyId,
            name,
            recipientsCount: recipients.length,
            timestamp: new Date().toISOString()
     ...
     */
    static async generateAiCopy(companyId, topic, channel) {
        try {
