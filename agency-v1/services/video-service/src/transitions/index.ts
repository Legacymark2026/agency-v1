...
const TRANSITION_RULES: Array<{
  fromCondition: (meta: ClipMetadata) => boolean;
  toCondition: (meta: ClipMetadata) => boolean;
...
export function getTransitionsByComplexity(
  complexity: TransitionPreset['complexity'],
): TransitionPreset[] {
  return TRANSITION_PRESETS.filter((p) => p.complexity === complexity);
}

export function suggestTransition(
  fromClip: ClipMetadata,
  toClip: ClipMetadata,
  options: TransitionOptions = {},
): TransitionSuggestion[] {
  const suggestions: TransitionSuggestion[] = [];
  const appliedTypes = new Set<TransitionType>();
...
export function autoSuggestAllTransitions(
  clips: ClipMetadata[],
  options: TransitionOptions = {},
): TransitionSuggestion[] {
  const allSuggestions: TransitionSuggestion[] = [];
...
export function analyzeClipForTransition(
  clipPath: string,
  clipId: string,
...
export function getTransitionFFmpegFilter(
  type: TransitionType,
  duration: number,
  parameters?: Record<string, any>,
): string {
