...
async function getOrCreateCompanyIdForUser(userId) {
    const companyUser = await database_1.prisma.companyUser.findFirst({
        where: { userId },
    ...
    const firstCompany = await database_1.prisma.company.findFirst();
    let companyId = firstCompany?.id;
    if (!companyId) {
        const newCompany = await database_1.prisma.company.create({
            data: { name: "Default Company", slug: "default-company" },
            select: { id: true }
        });
        companyId = newCompany.id;
    }
    const existingLink = await database_1.prisma.companyUser.findUnique({
        where: {
            userId_companyId: {
                userId,
                companyId
...
async function migrateLegacyConfig(companyId, oldProvider, oldData) {
    ...
    try {
        ...
        if (oldProvider === 'facebook' && (oldData.appId || oldData.appSecret)) {
            ...
            await database_1.prisma.integrationConfig.upsert({
...
app.delete('/api/integrations/accounts', async (req, res) => {
    try {
        ...
        const deleteAccountsResult = await database_1.prisma.account.deleteMany({
            where: {
                userId,
        ...
        if (companyId) {
            ...
            deleteConfigsResult = await database_1.prisma.integrationConfig.deleteMany({
                where: {
                    companyId,
                    provider: { in: providersToDelete }
                }
...
app.get('/api/integrations/config', async (req, res) => {
    try {
        ...
        let companyId = (req.query.companyId || req.body.companyId);
        ...
        if (legacyProvider) {
            const legacyConfig = await database_1.prisma.integrationConfig.findUnique({
        ...
        const config = await database_1.prisma.integrationConfig.findUnique({
            where: {
                companyId_provider: {
                    companyId,
                    provider
...
app.post('/api/integrations/config', async (req, res) => {
    try {
        ...
        const result = await database_1.prisma.integrationConfig.upsert({
...
app.post('/api/integrations/whatsapp', async (req, res) => {
    try {
        ...
        const result = await database_1.prisma.whatsAppIntegration.upsert({
            where: { phoneNumberId },
            update: {
                companyId,
                wabaId,
...
app.get('/api/integrations/domains/:domain', async (req, res) => {
    try {
        ...
        const verification = await database_1.prisma.emailDomainVerification.findUnique({
            where: { domain }
        });
...
app.post('/api/integrations/domains', async (req, res) => {
    try {
        ...
        const result = await database_1.prisma.emailDomainVerification.upsert({
            where: { domain },
