...
app.use((0, observability_1.metricsMiddleware)("goldneez-rewards-service"));
    ...
    try {
        ...
        if (rwdCheck.rows.length === 0) {
            ...
            for (const rwd of REWARDS_CATALOG) {
                await client.query(`INSERT INTO tbl_goldneez_rewards (id, title, cost, description, is_active, created_at)
           VALUES ($1, $2, $3, $4, true, NOW())`, [rwd.id, rwd.title, rwd.cost, rwd.desc]);
            }
        }
        // Sembrar eventos
        ...
        if (evtCheck.rows.length === 0) {
            ...
            for (const evt of MOCK_EVENTS) {
                await client.query(`INSERT INTO tbl_goldneez_events (id, title, description, date, time, capacity, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [evt.id, evt.title, evt.desc, evt.date, evt.time, evt.capacity]);
            }
        }
    }
    catch (err) {
...
async function getUserPointsAndMetadata(userId) {
    const result = await pool.query("SELECT id, metadata FROM tbl_user_profiles WHERE user_id = $1", [userId]);
    let points = 500;
    ...
    if (result.rows.length > 0) {
        ...
        await pool.query("INSERT INTO tbl_user_profiles (id, user_id, preferences, metadata) VALUES ($1, $2, $3, $4)", [profileId, userId, JSON.stringify({}), JSON.stringify(initialMeta)]);
    }
    return { points, registeredAt, city };
}
// Helper para guardar puntos en el perfil
async function updateUserPointsAndMetadata(userId, points, city, registeredAt) {
    const currentMeta = { points, registeredAt, city };
    await pool.query("UPDATE tbl_user_profiles SET metadata = $1 WHERE user_id = $2", [JSON.stringify(currentMeta), userId]);
}
// ── 1. Perfil de Sabor Endpoints ─────────────────────────────────────────────
app.get("/api/v1/goldneez-rewards/flavor-profile/:userId", async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await pool.query("SELECT * FROM tbl_goldneez_flavor_profiles WHERE user_id = $1", [userId]);
...
app.post("/api/v1/goldneez-rewards/flavor-profile", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            ...
            acidez: zod_1.z.number().min(1).max(5),
            cuerpo: zod_1.z.number().min(1).max(5),
            notas: zod_1.z.string(),
            metodoPreferido: zod_1.z.string(),
        });
        ...
        await pool.query(`INSERT INTO tbl_goldneez_flavor_profiles (id, user_id, acidez, cuerpo, notas, metodo_preferido, recomendaciones, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
       ON CONFLICT (user_id) DO UPDATE 
       SET acidez = EXCLUDED.acidez, cuerpo = EXCLUDED.cuerpo, notas = EXCLUDED.notas, 
           metodo_preferido = EXCLUDED.metodo_preferido, recomendaciones = EXCLUDED.recomendaciones, updated_at = NOW()`, [crypto_1.default.randomUUID(), userId, acidez, cuerpo, notas, metodoPreferido, recomendaciones]);
...
app.get("/api/v1/goldneez-rewards/rewards", async (_req, res) => {
    try {
        const result = await pool.query("SELECT id, title, cost, description FROM tbl_goldneez_rewards WHERE is_active = true ORDER BY cost ASC");
...
app.get("/api/v1/goldneez-rewards/points/:userId", async (req, res) => {
    try {
        ...
        // Obtener historial
        const historyResult = await pool.query("SELECT * FROM tbl_goldneez_points_history WHERE user_id = $1 ORDER BY created_at DESC", [userId]);
        let tier = "Silver";
        if (points >= 3000) {
            tier = "Platinum";
        }
...
async function dispatchLoyaltyNotification(userId, title, message) {
    try {
...
app.post("/api/v1/goldneez-rewards/points/add", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            userId: zod_1.z.string().uuid(),
            puntos: zod_1.z.number().positive(),
        ...
        await pool.query("INSERT INTO tbl_goldneez_points_history (id, user_id, puntos, tipo, concepto, created_at) VALUES ($1, $2, $3, $4, $5, NOW())", [crypto_1.default.randomUUID(), userId, puntos, "earned", concepto]);
        // Trigger notification if upgraded
        if (oldTier !== newTier && newTier !== "Silver") {
            await dispatchLoyaltyNotification(userId, `🎉 ¡Felicidades! Subiste a nivel ${newTier}`, `Has alcanzado el nivel ${newTier} en el Club Goldneez. ¡Disfruta de tus nuevos beneficios!`);
        }
...
app.post("/api/v1/goldneez-rewards/points/redeem", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            userId: zod_1.z.string().uuid(),
            puntosRequeridos: zod_1.z.number().positive(),
            concepto: zod_1.z.string(),
        });
        ...
        await updateUserPointsAndMetadata(userId, newPoints, city, registeredAt);
        // Insertar en historial
        await pool.query("INSERT INTO tbl_goldneez_points_history (id, user_id, puntos, tipo, concepto, created_at) VALUES ($1, $2, $3, $4, $5, NOW())", [crypto_1.default.randomUUID(), userId, -puntosRequeridos, "redeemed", concepto]);
        // Trigger notification if downgraded
        if (oldTier !== newTier) {
            await dispatchLoyaltyNotification(userId, `⚠️ Tu nivel de socio cambió a ${newTier}`, `Tu nivel en el Club Goldneez ahora es ${newTier} debido al canje de puntos.`);
        }
        res.json({ success: true, points: newPoints });
    }
    catch (err) {
...
app.post("/api/v1/goldneez-rewards/coupons/generate", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            ...
            lowActivity: zod_1.z.boolean().optional()
        });
        ...
        await pool.query("INSERT INTO tbl_goldneez_referrals (id, referrer_id, codigo, estado, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())", [crypto_1.default.randomUUID(), userId, code, "coupon"]);
        res.json({
            success: true,
            couponCode: code,
...
app.get("/api/v1/goldneez-rewards/referrals/:userId", async (req, res) => {
    try {
        ...
        let referralResult = await pool.query("SELECT codigo FROM tbl_goldneez_referrals WHERE referrer_id = $1 LIMIT 1", [userId]);
        let codigo = "";
        if (referralResult.rows.length > 0) {
            ...
            await pool.query("INSERT INTO tbl_goldneez_referrals (id, referrer_id, codigo, estado, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())", [crypto_1.default.randomUUID(), userId, codigo, "pending"]);
        }
        // Calcular estadísticas reales
        const statsResult = await pool.query("SELECT COUNT(*) as count FROM tbl_goldneez_referrals WHERE referrer_id = $1 AND estado = 'completed'", [userId]);
...
app.post("/api/v1/goldneez-rewards/referrals/apply", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            userId: zod_1.z.string().uuid(),
            codigo: zod_1.z.string().trim(),
        });
        const { userId, codigo } = schema.parse(req.body);
        // Validar código existente
        const referralResult = await pool.query("SELECT referrer_id, id FROM tbl_goldneez_referrals WHERE codigo = $1 LIMIT 1", [codigo]);
        if (referralResult.rows.length === 0) {
        ...
        const alreadyReferred = await pool.query("SELECT id FROM tbl_goldneez_referrals WHERE referred_id = $1 LIMIT 1", [userId]);
        ...
        const newRefId = crypto_1.default.randomUUID();
        await pool.query("INSERT INTO tbl_goldneez_referrals (id, referrer_id, referred_id, codigo, estado, puntos_referente, puntos_referido, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())", [newRefId, referrerId, userId, codigo, "completed", 200, 100]);
        // Otorgar puntos
        ...
        await pool.query("INSERT INTO tbl_goldneez_points_history (id, user_id, puntos, tipo, concepto, created_at) VALUES ($1, $2, $3, $4, $5, NOW())", [crypto_1.default.randomUUID(), userId, 100, "earned", `Código de referido aplicado (${codigo})`]);
        ...
        await pool.query("INSERT INTO tbl_goldneez_points_history (id, user_id, puntos, tipo, concepto, created_at) VALUES ($1, $2, $3, $4, $5, NOW())", [crypto_1.default.randomUUID(), referrerId, 200, "earned", `Amigo referido se ha registrado (${codigo})`]);
...
app.get("/api/v1/goldneez-rewards/events", async (_req, res) => {
    try {
        ...
        const eventsResult = await pool.query("SELECT id, title, description, date, time, capacity FROM tbl_goldneez_events ORDER BY created_at ASC");
        // Obtener reservas activas
        const bookings = await pool.query("SELECT event_id, COUNT(*) as count FROM tbl_goldneez_event_bookings WHERE estado = 'booked' GROUP BY event_id");
        const counts = bookings.rows.reduce((acc, row) => {
...
app.get("/api/v1/goldneez-rewards/events/bookings/:userId", async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await pool.query("SELECT * FROM tbl_goldneez_event_bookings WHERE user_id = $1 AND estado = 'booked' ORDER BY created_at DESC", [userId]);
        res.json(result.rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post("/api/v1/goldneez-rewards/events/book", async (req, res) => {
    try {
        ...
        const checkBooking = await pool.query("SELECT id FROM tbl_goldneez_event_bookings WHERE user_id = $1 AND event_id = $2 AND estado = 'booked'", [userId, eventId]);
        if (checkBooking.rows.length > 0) {
            return res.status(400).json({ error: "Ya tienes una reserva para este evento." });
        }
        const bookingId = crypto_1.default.randomUUID();
        await pool.query("INSERT INTO tbl_goldneez_event_bookings (id, user_id, event_id, event_title, event_date, estado, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())", [bookingId, userId, eventId, eventTitle, eventDate, "booked"]);
...
app.post("/api/v1/goldneez-rewards/events/cancel", async (req, res) => {
    try {
        ...
        await pool.query("UPDATE tbl_goldneez_event_bookings SET estado = 'cancelled', updated_at = NOW() WHERE id = $1", [bookingId]);
...
app.get("/api/v1/goldneez-rewards/shipping-tracking/:orderId", async (req, res) => {
    try {
        ...
        try {
            ...
            const dealResult = await pool.query("SELECT created_at FROM tbl_deals WHERE id::text LIKE $1 LIMIT 1", [`${shortId}%`]);
        ...
        for (const step of computedSteps) {
            await pool.query(`INSERT INTO tbl_goldneez_shipping_steps (id, order_id, step_index, label, status, desc, date, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
         ON CONFLICT (order_id, step_index) DO UPDATE
         SET status = EXCLUDED.status, date = EXCLUDED.date`, [crypto_1.default.randomUUID(), orderId, step.stepIndex, step.label, step.status, step.desc, step.date]);
        }
...
app.get("/api/v1/goldneez-rewards/reviews/:coffeeId", async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT * FROM tbl_goldneez_product_reviews WHERE coffee_id = $1 ORDER BY created_at DESC", [coffeeId]);
        res.json(result.rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post("/api/v1/goldneez-rewards/reviews", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            ...
            coffeeId: zod_1.z.string(),
            coffeeName: zod_1.z.string(),
            calificacion: zod_1.z.number().min(1).max(5),
        ...
        await pool.query("INSERT INTO tbl_goldneez_product_reviews (id, user_id, order_id, coffee_id, coffee_name, calificacion, comentario, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())", [reviewId, userId, orderId, coffeeId, coffeeName, calificacion, comentario || null]);
...
app.get("/api/v1/goldneez-rewards/notifications/:userId", async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await pool.query("SELECT * FROM tbl_goldneez_notification_preferences WHERE user_id = $1 LIMIT 1", [userId]);
        if (result.rows.length === 0) {
            const defaultPrefs = {
                promociones: true,
                pedidos: true,
...
app.post("/api/v1/goldneez-rewards/notifications", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            userId: zod_1.z.string().uuid(),
            promociones: zod_1.z.boolean(),
            pedidos: zod_1.z.boolean(),
            clubPuntos: zod_1.z.boolean(),
            boletines: zod_1.z.boolean(),
        });
        const { userId, promociones, pedidos, clubPuntos, boletines } = schema.parse(req.body);
        await pool.query(`INSERT INTO tbl_goldneez_notification_preferences (id, user_id, promociones, pedidos, club_puntos, boletines, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
       ON CONFLICT (user_id) DO UPDATE 
       SET promociones = EXCLUDED.promociones, pedidos = EXCLUDED.pedidos, club_puntos = EXCLUDED.club_puntos, 
           boletines = EXCLUDED.boletines, updated_at = NOW()`, [crypto_1.default.randomUUID(), userId, promociones, pedidos, clubPuntos, boletines]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
app.get("/api/v1/goldneez-rewards/payment-methods/:userId", async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await pool.query("SELECT * FROM tbl_goldneez_payment_methods WHERE user_id = $1 ORDER BY es_predeterminada DESC, created_at DESC", [userId]);
        res.json(result.rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post("/api/v1/goldneez-rewards/payment-methods", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            ...
            ultimosCuatro: zod_1.z.string().length(4),
            nombreTarjeta: zod_1.z.string(),
        });
        const { userId, marca, ultimosCuatro, nombreTarjeta } = schema.parse(req.body);
        const methodId = crypto_1.default.randomUUID();
        const checkFirst = await pool.query("SELECT id FROM tbl_goldneez_payment_methods WHERE user_id = $1 LIMIT 1", [userId]);
        const esPredeterminada = checkFirst.rows.length === 0;
        await pool.query("INSERT INTO tbl_goldneez_payment_methods (id, user_id, marca, ultimos_cuatro, nombre_tarjeta, token, es_predeterminada, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())", [methodId, userId, marca, ultimosCuatro, nombreTarjeta, `tok_${Math.random().toString(36).substring(7)}`, esPredeterminada]);
...
app.delete("/api/v1/goldneez-rewards/payment-methods/:userId/:methodId", async (req, res) => {
    try {
        ...
        await pool.query("DELETE FROM tbl_goldneez_payment_methods WHERE id = $1 AND user_id = $2", [methodId, userId]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get("/api/v1/goldneez-rewards/consumption/:userId", async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT details, created_at FROM tbl_user_activity_logs WHERE user_id = $1 AND action = 'CHECKOUT_SUCCESS' ORDER BY created_at ASC", [userId]);
        const months = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];
        const consumptionMap = months.reduce((acc, m) => {
            acc[m] = 0;
            return acc;
        }, {});
...
app.get("/api/v1/goldneez-rewards/subscription/:userId", async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await pool.query("SELECT preferences FROM tbl_user_profiles WHERE user_id = $1", [userId]);
        if (result.rows.length > 0) {
...
app.post("/api/v1/goldneez-rewards/subscription", async (req, res) => {
    try {
        const schema = zod_1.z.object({
            userId: zod_1.z.string().uuid(),
            beans: zod_1.z.string(),
            frequency: zod_1.z.string(),
            status: zod_1.z.string(),
            nextDelivery: zod_1.z.string(),
        });
        const { userId, beans, frequency, status, nextDelivery } = schema.parse(req.body);
        const result = await pool.query("SELECT id, preferences FROM tbl_user_profiles WHERE user_id = $1", [userId]);
        let currentPrefs = {};
        if (result.rows.length > 0) {
            const prefsStr = result.rows[0].preferences;
            if (prefsStr) {
        ...
        if (result.rows.length > 0) {
            await pool.query("UPDATE tbl_user_profiles SET preferences = $1 WHERE user_id = $2", [JSON.stringify(updatedPrefs), userId]);
        }
        else {
            const profileId = crypto_1.default.randomUUID();
            await pool.query("INSERT INTO tbl_user_profiles (id, user_id, preferences, metadata) VALUES ($1, $2, $3, $4)", [profileId, userId, JSON.stringify(updatedPrefs), JSON.stringify({})]);
        }
