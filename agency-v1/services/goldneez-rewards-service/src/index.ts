...
app.post("/api/goldneez-rewards/flavor-profile", async (req, res) => {
  try {
    ...
    const notasList = notas.toLowerCase().split(",").map((n) => n.trim());
    let recs: string[] = [];
...
// ── 2. Puntos e Historial Endpoints ──────────────────────────────────────────
...
app.get("/api/goldneez-rewards/referrals/:userId", async (req, res) => {
  try {
    ...
    const pointsEarned = count * 200; // 200 puntos por referido completado
...
app.post("/api/goldneez-rewards/referrals/apply", async (req, res) => {
  try {
    ...
    // 1. Validar si el código existe y obtener el referente (referrer)
    ...
    const { referrer_id: referrerId, id: refId } = referralResult.rows[0];

    if (referrerId === userId) {
      return res.status(400).json({ error: "No puedes aplicar tu propio código." });
    }

    // 2. Verificar que el usuario no haya sido referido previamente
    ...
    // 3. Registrar la aplicación completada
    ...
    // 4. Agregar puntos a ambos usuarios
    // Referido (+100 puntos)
...
app.get("/api/goldneez-rewards/events", async (_req, res) => {
  try {
    // Para simplificar, obtenemos la lista y cruzamos con las reservas reales para actualizar aforo
    ...
    const eventsList = MOCK_EVENTS.map((evt) => {
...
app.post("/api/goldneez-rewards/events/book", async (req, res) => {
  try {
    ...
    // Validar si ya tiene reserva activa para este evento
...
// ── 5. Seguimiento de Envíos (Visual Tracking) ──────────────────────────────
const DEFAULT_STEPS = [
...
app.get("/api/goldneez-rewards/shipping-tracking/:orderId", async (req, res) => {
  try {
    ...
    // En un escenario real, buscaríamos en BD. Para simplificar, devolvemos estados dinámicos según el ID de la orden
...
app.post("/api/goldneez-rewards/payment-methods", async (req, res) => {
  try {
    ...
    // Si es la primera, marcarla como predeterminada
...
// ── 9. Consumo Mensual (Consumo de café en gramos) ───────────────────────────
app.get("/api/goldneez-rewards/consumption/:userId", async (req, res) => {
  try {
    const { userId } = req.params;

    // En un escenario real, calculamos basado en compras. Mokeamos las cantidades de café en gramos compradas de Ene a Dic
    const consumption = [
      { month: "Ene", grams: 500 },
      { month: "Feb", grams: 250 },
      { month: "Mar", grams: 750 },
