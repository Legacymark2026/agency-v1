...
export class BruteForceService {
   ...
   */
  static async checkLockout(ip: string, email: string) {
    if (!redis || redis.status !== 'ready') return { isLocked: false };
   ...
   */
  static async recordFailedAttempt(ip: string, email: string) {
   ...
   */
  static async resetLockout(ip: string, email: string) {
    if (!redis || redis.status !== 'ready') return;
