...
const requireAuth = async (req, res, next) => {
    try {
        ...
        if (gatewayUserId) {
            req.authUser = { id: String(gatewayUserId) };
            return next();
        }
        ...
        req.authUser = { id: decoded.sub, role: decoded.role, companyId: decoded.companyId };
        next();
    }
    catch {
...
app.use("/api/v1", notification_routes_1.notificationRouter);
...
app.get("/api/notifications", requireAuth, async (req, res) => {
    try {
        ...
        const unreadCount = await (0, notification_cache_1.getUnreadCountCached)(targetUserId, String(companyId || ""));
        res.json({
            notifications,
            total,
            unreadCount,
...
app.get("/api/notifications/stats", async (req, res) => {
    try {
        ...
        const dlqStats = await (0, notification_queue_1.getDLQStats)();
        res.json({
            period,
            total,
            unread,
...
app.get("/api/v1/notifications/dlq", requireAuth, async (req, res) => {
    try {
        ...
        const jobs = await (0, notification_queue_1.getDLQJobs)(parseInt(String(start)), parseInt(String(end)));
...
const notifGrpcServer = new grpc_1.GrpcServerHelper();
notifGrpcServer.addService(grpc_1.PROTO_PATHS.notification, "notification", "NotificationService", {
    SendDirectNotification: async (call, callback) => {
        try {
