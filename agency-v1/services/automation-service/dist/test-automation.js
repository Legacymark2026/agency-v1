    ...
    try {
        ...
        const deal = await database_1.prisma.deal.create({
            data: {
                ...
                priority: "MEDIUM",
                companyId: companyId,
            } // Use any if type schema has other optional/required relations
        });
        dealId = deal.id;
        ...
        await (0, workflow_executor_1.executeWorkflow)(workflowId, context1);
        // Obtener la ejecución
        const execution1 = await database_1.prisma.workflowExecution.findFirst({
            where: { workflowId },
            orderBy: { startedAt: 'desc' }
        });
        console.log(`   Estado de la ejecución: ${execution1?.status}`);
        const logs1 = execution1?.logs || [];
        ...
        const updatedDeal = await database_1.prisma.deal.findUnique({ where: { id: dealId } });
        ...
        await (0, workflow_executor_1.executeWorkflow)(workflowId, context2);
        const execution2 = await database_1.prisma.workflowExecution.findFirst({
            where: { workflowId },
            orderBy: { startedAt: 'desc' }
        });
        console.log(`   Estado de la ejecución: ${execution2?.status}`);
        const logs2 = execution2?.logs || [];
