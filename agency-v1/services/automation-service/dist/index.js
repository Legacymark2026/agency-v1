...
app.post("/api/workflows", async (req, res) => {
    try {
        ...
        let workflow;
        if (data.id) {
            workflow = await database_1.prisma.workflow.update({
                where: { id: data.id },
                data: {
                    name: data.name,
            ...
            workflow = await database_1.prisma.workflow.create({
                data: {
                    companyId,
                    name: data.name,
                    triggerType: data.triggerType,
...
app.post("/api/workflows/:id/toggle", async (req, res) => {
    try {
        ...
        const workflow = await database_1.prisma.workflow.update({
            where: { id: req.params.id },
...
app.post("/api/workflows/bulk-toggle", async (req, res) => {
    try {
        ...
        if (!Array.isArray(ids))
            return res.status(400).json({ error: "ids must be an array" });
        await database_1.prisma.workflow.updateMany({
            where: { id: { in: ids } },
            data: { isActive: Boolean(isActive) },
        });
        res.json({ success: true });
...
app.get("/api/automation/analytics", async (req, res) => {
    try {
        ...
        const [totalWorkflows, activeWorkflows, totalExecutions, failedExecutions] = await Promise.all([
            database_1.prisma.workflow.count({ where: { companyId: String(companyId) } }),
            database_1.prisma.workflow.count({ where: { companyId: String(companyId), isActive: true } }),
            database_1.prisma.workflowExecution.count({ where: { workflow: { companyId: String(companyId) } } }),
            database_1.prisma.workflowExecution.count({ where: { workflow: { companyId: String(companyId) }, status: "FAILED" } })
        ]);
        ...
        const recentActivity = await database_1.prisma.workflowExecution.groupBy({
            by: ["status"],
            where: {
        ...
        const topWorkflows = await database_1.prisma.workflow.findMany({
            where: { companyId: String(companyId) },
            include: {
                _count: {
                    select: { executions: true }
...
app.get("/api/automation/integrations-status", async (req, res) => {
    try {
        ...
        if (!companyId)
            return res.status(400).json({ error: "companyId required" });
        const configs = await database_1.prisma.integrationConfig.findMany({
            where: { companyId: String(companyId) }
        });
        const statusMap = {};
        ...
        const wa = await database_1.prisma.whatsAppIntegration.findFirst({
...
app.post("/api/workflows/trigger", async (req, res) => {
    try {
        ...
        if (!triggerType)
            return res.status(400).json({ error: "triggerType required" });
        const result = await (0, workflow_executor_1.triggerWorkflow)(triggerType, triggerData);
...
app.post("/api/workflows/:id/execute", async (req, res) => {
    try {
        ...
        (0, workflow_executor_1.executeWorkflow)(req.params.id, triggerData || {}, resumeFromNodeId)
...
app.post("/api/workflows/:id/trigger", async (req, res) => {
    try {
        ...
        (0, workflow_executor_1.executeWorkflow)(req.params.id, triggerData || {})
            .catch(err => console.error(`[automation-service] Async execute/resume error for ${req.params.id}:`, err));
        res.json({ success: true, message: "Execution started in background" });
    }
    catch (err) {
...
app.post("/api/workflows/executions/:id/resume", async (req, res) => {
    try {
        ...
        (0, workflow_executor_1.executeWorkflow)(execution.workflowId, {}, resumeFromNodeId)
...
eventBus.subscribe("deal.stage_changed", async (payload) => {
    ...
    try {
        const result = await (0, workflow_executor_1.triggerWorkflow)("DEAL_STAGE_CHANGED", {
            id: data.dealId,
            dealId: data.dealId,
            stage: data.stage,
