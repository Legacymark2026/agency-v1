...
exports.InboxAnalysisService = void 0;
...
class InboxAnalysisService {
    ...
    static async analyzeSentiment(content) {
        ...
        negativeWords.forEach(w => { if (text.includes(w))
            negativeCount++; });
        angryWords.forEach(w => { if (text.includes(w))
            angryCount++; });
        if (angryCount > 0) {
     ...
     */
    static async generateSuggestedReply(conversationId) {
        console.log(`[InboxAnalysisService] Generating suggested reply for conversation: ${conversationId}`);
        let lastMessages = [];
        try {
            lastMessages = await database_1.prisma.message.findMany({
...
exports.InboxAnalysisService = InboxAnalysisService;
//# sourceMappingURL=inbox-analysis.service.js.map