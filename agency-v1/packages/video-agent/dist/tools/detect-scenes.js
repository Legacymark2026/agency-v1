export async function detectVisualScenes(videoUrl, options) {
    const threshold = (options === null || options === void 0 ? void 0 : options.threshold) || 0.3;
    const minSceneLength = (options === null || options === void 0 ? void 0 : options.minSceneLength) || 0.5;
...
async function analyzeVideoFrames(videoUrl, threshold, minSceneLength) {
    const scenes = [];
    let timestamp = 0;
    let lastSceneTime = 0;
    while (timestamp < 60) {
...
function generateMockVisualAnalysis() {
    return {
        scenes: [
