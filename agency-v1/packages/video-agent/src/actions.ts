 ...
 */
export function createVideoCoordinator(
  companyId: string,
  apiKey: string
): AgentCoordinator {
 ...
 */
export async function runFullEditWorkflow(
  companyId: string,
  apiKey: string,
...
export async function runSpecificAgent(
  companyId: string,
  apiKey: string,
  agentName: 'logos' | 'croma' | 'phonos' | 'graphos',
  input: any
  ...
  try {
    ...
    return { success: true, result: { agent: agentName, output: result as any } };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
}
