...
export class StyleMatcher {
  ...
  analyzeSourceVideo(clip: VideoClip): StyleProfile {
    ...
    const qualityGrainMap: Record<string, number> = {
      'excellent': 15,
      'good': 25,
      'fair': 40,
      'poor': 60
    ...
    const lighting = (clip as any).lighting || 'natural';
    const quality = (clip as any).quality || 'good';
   ...
   */
  private calculateGrainAnimation(level: number): GrainAnimationConfig {
    return {
      enabled: level > 10,
  ...
  generateLUTPreset(profile: StyleProfile): string {
    ...
    if (profile.temperature < 4500) return 'warm-golden-hour';
    if (profile.temperature > 6000) return 'cool-blue-hour';
    if (profile.contrast > 1.3) return 'cinematic-high-contrast';
    if (profile.saturation > 1.1) return 'vibrant-pop';
  ...
  validateMatch(
    ...
    const matchScore = 1 - (
      (tempDiff / 2000) * 0.3 +  // 30% peso en temperatura
