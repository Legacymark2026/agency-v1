...
export class SynthesisAgent {
   ...
   */
  private async initGemini(apiKey: string): Promise<void> {
    if (!this.gemini) {
   ...
   */
  async runAudit(geminiApiKey: string): Promise<SynthesisAudit> {
   ...
   */
  private async detectGaps(): Promise<GapDetectionResult> {
   ...
   */
  private async detectGapsWithAI(): Promise<TimelineGap[]> {
...
`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return detected.map((d: any, i: number) => ({
          ...
          endTime: 3,
          duration: 3,
          type: d.type as GapType,
          severity: 'major' as GapSeverity,
          reason: d.reason,
   ...
   */
  private inferGapType(clipDuration: number): GapType {
    if (clipDuration < 5) return 'product';
    if (clipDuration < 10) return 'texture';
    if (clipDuration < 20) return 'b-roll';
    return 'background';
  }
   ...
   */
  private extractRelatedScript(start: number, end: number): string {
   ...
   */
  private async generateProposals(gaps: TimelineGap[]): Promise<AssetProposal[]> {
   ...
   */
  private determineBestSource(gap: TimelineGap): ProposedSource {
   ...
   */
  private generateSearchQuery(gap: TimelineGap): string {
    const typeQueries: Record<GapType, string> = {
   ...
   */
  private generateAIPrompt(gap: TimelineGap): string {
    const basePrompts: Record<GapType, string> = {
   ...
   * Estima costo de IA
   */
  private estimateAICost(type: GapType): number {
    const costs: Record<GapType, number> = {
      'b-roll': 20,
      'transition': 10,
      'texture': 5,
      'background': 15,
   ...
   */
  private async executeStockSearch(proposal: AssetProposal): Promise<any> {
   ...
   */
  private async executeAIGeneration(proposal: AssetProposal): Promise<any> {
