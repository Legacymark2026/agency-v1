...
const DEFAULT_OPTIONS: ConfidenceOptions = {
  minThreshold: 0.3,
  reviewThreshold: 0.7,
  factorWeights: {
...
export class ConfidenceScorer {
  private options: ConfidenceOptions;
  private historicalData: Map<string, HistoricalRecord[]> = new Map();
  ...
  score(
    ...
    const complexityScore = this.scoreComplexity(suggestion, context?.projectComplexity || 0.5);
    const riskScore = this.scoreRisk(suggestion, actionType);
    const userAffinity = this.scoreUserAffinity(suggestion, context?.userPreferences);
    ...
    const recommendation =
      overall >= (this.options.reviewThreshold || 0.7)
        ? 'accept'
  ...
  private scoreContextSimilarity(
    ...
    if (recentAccepted.length === 0) return 0.4;
  ...
  private computeSimilarity(
    ...
    for (const key of allKeys) {
      ...
      if (typeof a[key] === 'number' && typeof b[key] === 'number') {
        ...
        if (diff / maxVal < 0.2) matchCount++;
      } else if (
        typeof a[key] === 'string' &&
        typeof b[key] === 'string'
      ) {
        if (a[key] === b[key]) matchCount++;
  ...
  private scoreComplexity(
    ...
    const combinedComplexity =
      complexityFromParams * 0.4 + projectComplexity * 0.6;
  ...
  private scoreTemporalRelevance(
    ...
    if (recentRecords.length === 0) return 0.3;
  ...
  private getHistoricalReason(
    ...
    if (recordCount === 0) return 'Sin datos históricos disponibles';
    if (accuracy > 0.8) return `Alta precisión histórica: ${Math.round(accuracy * 100)}% de aceptación`;
    if (accuracy > 0.5) return `Precisión moderada: ${Math.round(accuracy * 100)}% de aceptación`;
    return `Baja precisión histórica: ${Math.round(accuracy * 100)}% de aceptación`;
  }

  private getContextReason(similarity: number): string {
    if (similarity > 0.8) return 'Muy similar a sugerencias aceptadas anteriormente';
    if (similarity > 0.5) return 'Moderadamente similar a sugerencias previas';
    return 'Poca similitud con sugerencias previas';
  }

  private getComplexityReason(score: number): string {
    if (score > 0.7) return 'Baja complejidad — bajo riesgo de errores';
    if (score > 0.4) return 'Complejidad moderada';
    return 'Alta complejidad — revisar cuidadosamente';
  }

  private getRiskReason(score: number): string {
    if (score > 0.7) return 'Bajo riesgo — edición segura';
    if (score > 0.4) return 'Riesgo moderado';
    return 'Alto riesgo — afecta múltiples elementos';
  }

  private getAffinityReason(score: number): string {
    if (score > 0.7) return 'Fuerte alineación con preferencias del usuario';
    if (score > 0.4) return 'Alineación moderada con preferencias';
    return 'Sin datos de preferencias del usuario';
  }

  private getTemporalReason(score: number): string {
    if (score > 0.7) return 'Basado en comportamiento reciente';
    if (score > 0.4) return 'Basado en datos históricos parciales';
...
export function createConfidenceScorer(options?: ConfidenceOptions): ConfidenceScorer {
