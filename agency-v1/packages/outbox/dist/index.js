...
exports.OutboxWorker = exports.OutboxWriter = exports.OUTBOX_SETUP_SQL = void 0;
const pg_1 = require("pg");
const ioredis_1 = __importDefault(require("ioredis"));
const uuid_1 = require("uuid");
// ── SQL for setup ─────────────────────────────────────────────────────────────
exports.OUTBOX_SETUP_SQL = `
  CREATE TABLE IF NOT EXISTS outbox_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    topic VARCHAR(100) NOT NULL,
    payload JSONB NOT NULL,
...
class OutboxWriter {
     ...
     */
    async enqueue(client, topic, payload, options) {
       ...
       VALUES ($1, $2, $3, $4, 'pending', 0, $5, $6)`, [id, topic, JSON.stringify(payload), this.serviceId, maxAttempts, scheduledAt]);
        return id;
    }
    /**
     ...
     */
    async setup(pool) {
        await pool.query(exports.OUTBOX_SETUP_SQL);
    }
}
exports.OutboxWriter = OutboxWriter;
// ── OutboxWorker ──────────────────────────────────────────────────────────────
/**
...
class OutboxWorker {
    constructor(options) {
        ...
        this.pool = new pg_1.Pool({ connectionString: options.connectionString });
        this.redis = new ioredis_1.default(options.redisUrl, { lazyConnect: true, enableReadyCheck: false });
        this.dlqRedis = new ioredis_1.default(options.redisUrl, { lazyConnect: true, enableReadyCheck: false });
    }
    /** Sets up the outbox table (idempotent). */
    async setup() {
        await this.pool.query(exports.OUTBOX_SETUP_SQL);
    ...
    async poll() {
        ...
        try {
            ...
            const { rows } = await client.query(`SELECT id, topic, payload, service_id, attempts, max_attempts
         FROM outbox_events
         ...
         LIMIT $2
         FOR UPDATE SKIP LOCKED`, [this.serviceId, this.batchSize]);
            if (rows.length === 0) {
            ...
            await client.query(`UPDATE outbox_events SET status = 'processing', attempts = attempts + 1
         WHERE id = ANY($1::uuid[])`, [ids]);
            ...
            for (const event of rows) {
                try {
                    ...
                    await this.pool.query(`UPDATE outbox_events
             ...
             WHERE id = $1`, [event.id]);
                    ...
                    if (nextAttempt >= (event.maxAttempts ?? this.maxAttempts)) {
                        ...
                        await this.pool.query(`UPDATE outbox_events SET status = 'dead' WHERE id = $1`, [event.id]);
    ...
    async moveToDLQ(event) {
        const dlqKey = `dead-letter:${this.serviceId}:${event.topic}`;
        await this.dlqRedis.lpush(dlqKey, JSON.stringify({
            ...event,
