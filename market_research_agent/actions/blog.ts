...
export async function getAllComments(companyId: string, options?: {
    ...
    try {
        ...
        // Get all posts for this company - try multiple approaches
        let posts = await prisma.post.findMany({
            where: { author: { companyId } },
            select: { id: true }
        });
        
        console.log('[getAllComments] posts found via author.companyId:', posts.length);
        
        // If no posts found, try getting all posts and filter manually
        if (posts.length === 0) {
