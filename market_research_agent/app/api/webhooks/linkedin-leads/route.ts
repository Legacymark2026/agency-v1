...
export async function POST(req: NextRequest) {
    try {
        ...
        const config = await getIntegrationConfig('linkedin-insight');
        const expectedKey = config?.linkedinWebhookKey;
        
        // Authorization could be in the header "Authorization: Bearer <key>" or as a query param
        const authHeader = req.headers.get('authorization')?.replace('Bearer ', '');
        const queryKey = req.nextUrl.searchParams.get('key');
        
        if (expectedKey && (authHeader !== expectedKey && queryKey !== expectedKey)) {
