      ...
      <div className="pl-2">
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded flex items-center gap-1" style={{ color: pCfg.color, background: pCfg.bg }}>
              <PIcon className="w-2.5 h-2.5" />{pCfg.label}
            </span>
            {task.labels?.slice(0, 2).map(label => (
              <span key={label} className="text-[9px] px-1.5 py-0.5 rounded font-semibold" style={{ color: "#fff", background: LABEL_COLORS[label] || "#64748b" }}>
                {label}
              </span>
            ))}
          </div>
          ...
          <>
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="flex items-center gap-1">
                {task.assignee ? (
                  <div className="w-6 h-6 rounded-full bg-teal-600 flex items-center justify-center text-[9px] font-bold text-white border border-slate-800 overflow-hidden">
                    {task.assignee.image ? (
                  ...
                  <div className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-[9px] text-slate-500 border border-slate-700">?</div>
                )}
              </div>
...
function QuickCreateModal({ swimlaneId, projectId, defaultStatus, onClose, onCreated }: {
  ...
  useEffect(() => {
    getTeamMembersForAssignment().then(res => { if (res.success) setUsers(res.users); });
  }, []);
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-4">
          {["URGENT","HIGH","MEDIUM","LOW"].map(p => {
            ...
            return (
              <button key={p} onClick={() => setPriority(p)} className="flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all" style={{ color: cfg.color, background: priority === p ? cfg.bg : "transparent", border: `1px solid ${priority === p ? cfg.color : "#334155"}` }}>
                {cfg.label}
              </button>
            );
          })}
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div>
            <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Fecha Límite</label>
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={() => setShowFilters(v => !v)} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors border ${showFilters || activeFilterCount > 0 ? "bg-teal-500/10 border-teal-500/30 text-teal-400" : "bg-slate-900 border-slate-800 text-slate-400 hover:bg-slate-800"}`}>
            <Filter className="w-3.5 h-3.5" />Filtros {activeFilterCount > 0 && <span className="bg-teal-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">{activeFilterCount}</span>}
          </button>
          ...
          <div className="flex items-center gap-4 p-3 bg-slate-900/60 border border-slate-800 rounded-xl flex-wrap">
            <div className="flex items-center gap-2">
              ...
              {["URGENT","HIGH","MEDIUM","LOW"].map(p => {
                ...
                return (
                  <button key={p} onClick={() => togglePriorityFilter(p)} className="text-[11px] px-2.5 py-1 rounded-full font-bold transition-all"
                ...
                return (
                  <div key={col.status} className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-mono px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">{total}</span>
                    </div>
                  </div>
                );
              })}
                ...
                <div key={swimlane.id} className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => toggleSwimlane(swimlane.id)} className={`flex items-center gap-2 px-4 py-1.5 rounded-full border transition-all text-sm font-semibold ${
                      ...
                      {swimlane.wipLimit && (
                        <span className={`text-[11px] px-1.5 py-0.5 rounded font-bold ${isOverWIP ? "bg-red-500/20 text-red-400" : "bg-slate-800 text-slate-400"}`}>
          ...
          {activeTask && (
            <div className="bg-slate-900 border border-teal-500 p-3 rounded-xl shadow-2xl shadow-teal-500/20 rotate-2 w-56 ring-1 ring-teal-500/30">
              ...
              <div className="mt-2 flex items-center gap-1.5">
                <span className="text-[10px] px-1.5 py-0.5 rounded text-teal-400 bg-teal-500/10 font-bold">{activeTask.priority}</span>
              </div>
            </div>
          )}
        </DragOverlay>
      ...
      {advancedCreate && (
        <AdvancedTaskCreateModal
          ...
          swimlanes={swimlanes}
          users={[]} // We will rely on getKanbanUsers within SwimlaneKanban to pass users, or let the modal fetch them. Wait, AdvancedTaskCreateModal doesn't fetch users right now!
