        ...
        <div className="space-y-4">
            ...
            {previewUrl && (
                <div className="grid grid-cols-2 gap-3 animate-in fade-in">
                    <div className="space-y-1">
                        <p className="text-[10px] font-mono text-slate-500">Actual</p>
                    ...
                    <div className="space-y-1">
                        <p className="text-[10px] font-mono text-violet-400">Vista previa v.anterior</p>
            ...
            {isLoading ? (
                ...
                <div className="border border-dashed border-slate-800 rounded-xl p-6 text-center">
                    ...
                    <p className="text-[10px] text-slate-600 mt-1">Cada regeneración guarda automáticamente una nueva versión.</p>
                </div>
            ) : (
                    ...
                    {versions.map((v) => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {v.changeNote && (
                                    <p className="text-[10px] text-slate-400 truncate mt-0.5">{v.changeNote}</p>
                                )}
                                {v.prompt && (
                                    <p className="text-[9px] text-slate-600 truncate font-mono mt-0.5">{v.prompt}</p>
                                )}
                                <p className="text-[9px] text-slate-600 mt-0.5">por {v.createdBy.name}</p>
                            </div>
                            <Button
                                size="sm"
                                variant="ghost"
                                ...
                                className="h-7 text-[10px] text-slate-500 hover:text-violet-400 hover:bg-violet-500/10 opacity-0 group-hover:opacity-100 transition-all font-mono gap-1"
                            >
                                {restoringId === v.id
