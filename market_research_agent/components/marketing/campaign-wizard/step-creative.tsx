...
import { Upload, Link2, Trash2, ExternalLink, Sparkles, Loader2 } from 'lucide-react';
...
export function StepCreative() {
    const { platforms, creative, name, objective, setCreative, addAssetUrl, removeAssetUrl, nextStep, prevStep } =
        useCampaignWizard();
    ...
    async function handleAIGenerate() {
        ...
        try {
            const prompt = `Campaña: ${name}. Objetivo: ${objective}. Redacta un titular corto y atractivo (max 40 chars) y una descripción persuasiva detallando los beneficios de nuestro servicio/producto según el objetivo mencionado.`;
            ...
            if (res.success && res.data) {
                ...
                if (generated) {
                    const newHeadlines = [...(creative.headlines || [])];
                    const newDescriptions = [...(creative.descriptions || [])];
                    if (newHeadlines.length === 0 || !newHeadlines[0]) newHeadlines[0] = generated.substring(0, 40);
                    else newHeadlines.push(generated.substring(0, 40));
                    
                    if (newDescriptions.length === 0 || !newDescriptions[0]) newDescriptions[0] = generated.substring(40, 165);
                    else newDescriptions.push(generated.substring(40, 165));

                    setCreative({
                        headlines: newHeadlines,
                        descriptions: newDescriptions,
                    });
                    toast.success("✨ Copy generado con IA correctamente");
                }
            } else {
